/**
 * Gondolin micro-VM sandbox — escalation tier (engaged via `--sandbox`).
 *
 * Routes all built-in tools (read, write, edit, bash, grep, find, ls) and
 * `!` user-bash commands into a local Gondolin micro-VM. The host working
 * directory is mounted at /workspace in the guest; file changes under
 * /workspace write through to the host. Sibling workspaces can be added as
 * additional VFS mounts via the `extraMounts` config field, so the agent can
 * reference workspace-B while running in workspace-A without the
 * permission-system `external_directory: ask` gate firing.
 *
 * Adapted from pi's examples/extensions/gondolin. Loaded dynamically by
 * sandbox/index.ts when `--sandbox` is passed. If `@earendil-works/gondolin`
 * is missing (optional dependency, install failed, or platform unsupported),
 * the dynamic import in the loader fails and it falls back to bubblewrap with
 * a loud warning.
 *
 * Requirements at runtime: Node.js >= 23.6.0 for @earendil-works/gondolin,
 * plus QEMU (install through your package manager).
 *
 * Config files (merged, project takes precedence):
 * - ~/.pi/agent/extensions/sandbox-gondolin.json (global)
 * - <cwd>/.pi/sandbox-gondolin.json (project-local)
 */
...
export interface GondolinConfig {
	enabled?: boolean;
	/** Extra host paths to mount inside the VM besides the workspace cwd. */
	extraMounts?: Array<{ host: string; guest: string }>;
}

const DEFAULT_CONFIG: GondolinConfig = {
	enabled: true,
	extraMounts: [],
};
...
function loadConfig(cwd: string): GondolinConfig {
	const projectConfigPath = join(cwd, ".pi", "sandbox-gondolin.json");
	const globalConfigPath = join(getAgentDir(), "extensions", "sandbox-gondolin.json");

	let globalConfig: Partial<GondolinConfig> = {};
	let projectConfig: Partial<GondolinConfig> = {};
	...
	return {
		enabled: projectConfig.enabled ?? globalConfig.enabled ?? DEFAULT_CONFIG.enabled ?? true,
		extraMounts: [...(globalConfig.extraMounts ?? []), ...(projectConfig.extraMounts ?? [])],
	};
}
...
async function executeGondolinGrep(
	...
	if (truncation.truncated) {
		...
		notices.push(`${formatSizeLocal(DEFAULT_MAX_BYTES)} limit reached`);
	}
...
/**
 * Engage the Gondolin micro-VM for full-tool isolation. Returns true if the VM
 * started and all path tools + bash were replaced; false if it could not engage
 * (config disabled, VM creation failed).
 *
 * Assumes `@earendil-works/gondolin` is installed (the loader's dynamic import
 * gated that). Called from sandbox/index.ts session_start when `--sandbox` set.
 */
export async function setupGondolin(pi: ExtensionAPI, ctx: ExtensionContext): Promise<boolean> {
	...
	for (const m of config.extraMounts ?? []) {
		mounts[m.guest] = new RealFSProvider(m.host);
	}
	...
	try {
		...
		const bashProbe = await vm.exec(["/bin/sh", "-lc", "command -v bash || true"]);
		shellPath = bashProbe.stdout.trim() || "/bin/sh";
		ctx.ui.notify(`Gondolin VM ready. ${localCwd} mounted at ${GUEST_WORKSPACE}.`, "info");
	} catch (err) {
		ctx.ui.notify(
			`Gondolin VM failed to start: ${err instanceof Error ? err.message : err}. Requires QEMU + Node >= 23.6. Falling back.`,
			"error",
		);
		return false;
	}
	...
	pi.on("session_shutdown", async (_event, shutdownCtx) => {
		shutdownCtx.ui.setStatus("sandbox", shutdownCtx.ui.theme.fg("muted", "Gondolin: stopping"));
		try {
			await activeVm.close();
		} finally {
			shutdownCtx.ui.setStatus("sandbox", undefined);
		}
	});
		...
		handler: async (_args, cmdCtx) => {
			cmdCtx.ui.notify(
				[
					...
					`Shell: ${shellPath}`,
					`Extra mounts: ${(config.extraMounts ?? []).map((m) => `${m.host} -> ${m.guest}`).join(", ") || "(none)"}`,
				].join("\n"),
				"info",
			);
		},
