...
const DEFAULT_MODEL_PREFERENCES: ModelPreference[] = [
...
function loadAnswerConfig(): AnswerConfig {
	const configPath = join(getAgentDir(), "answer.json");
	if (!existsSync(configPath)) return {};
	try {
		return JSON.parse(readFileSync(configPath, "utf-8")) as AnswerConfig;
	} catch {
		return {};
	}
}

function getExtractionModels(): ModelPreference[] {
	const config = loadAnswerConfig();
	if (config.extractionModels && config.extractionModels.length > 0) {
		return config.extractionModels;
	}
...
async function selectExtractionModel(
	...
	const preferences = getExtractionModels();

	for (const pref of preferences) {
		const model = modelRegistry.find(pref.provider, pref.modelId);
		if (model) {
	...
	// Fallback: try current model
	const auth = await modelRegistry.getApiKeyAndHeaders(currentModel);
	if (auth.ok) {
		return currentModel;
	}

	throw new Error("No available model for question extraction. Configure models in ~/.pi/agent/answer.json");
}
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			try {
				extractionModel = await selectExtractionModel(ctx.model, ctx.modelRegistry);
			} catch (error) {
